...
deseq2_compare = function(deseq_dataset, contrast = NULL, name = NULL) {
  ...
  if(!is.null(contrast)) {
    ...
    res = results(deseq_dataset, contrast = contrast, cooksCutoff = FALSE)
    # res.apeglm=lfcShrink(deseq_dataset, coef = "group_CPR_40_vs_CPR_0",type="apeglm")
    ...
    res = results(deseq_dataset, name = name, cooksCutoff = FALSE)
    res_name = name
  }
  ...
  # plot_volcano(stats_df = res_df,  positive_group=contrast[2],negative_group=contrast[3] , gene_col = "gene", fc_col = "log2FC", p_col = "padj", p_cutoff = 0.05,
  ...
  for (i in 1:length(hmg)) {
    ...
    hm_file_prefix = glue("{heatmaps_dir}/heatmap_{file_suffix}_{hmg[[i]]$file_suffix}")
