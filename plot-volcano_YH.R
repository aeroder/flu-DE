...
plot_volcano = function(stats_df,
  ...
  volcano_df$sig<-"not_significant"
  volcano_df$sig[volcano_df$fc < (-fc_cutoff) & volcano_df$p < p_cutoff]<-negative_group
  volcano_df$sig[volcano_df$fc > (fc_cutoff) & volcano_df$p < p_cutoff]<-positive_group
  volcano_df$sig<-factor(volcano_df$sig, levels = c("not_significant",negative_group, positive_group ))

  volcano_df$p[volcano_df$p==0 & !is.na(volcano_df$p)]<-min(volcano_df$p[volcano_df$p!=0&!is.na(volcano_df$p)])*1e-2
  ...
  volcano_plot = ggplot(volcano_df, aes(x = fc, y = -log10(p), color = sig)) +
    ...
    ggtitle(title) +
    scale_color_manual(values = c("slategrey", "blue","firebrick2"))
